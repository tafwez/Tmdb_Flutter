...
import 'package:tmdb_app/data/models/tv_cast_crew_model.dart';
...
class TvDetails extends StatefulWidget {
  ...
  const TvDetails({super.key, required this.id, required this.title});
...
class _TvDetailsState extends State<TvDetails> {
  ...
  TvCastCrewModel? credits;

  @override
  void initState() {
    super.initState();
    _detailsCubit = getIt<MainCubit>()..getTvDetails(int.parse(widget.id));
    _creditsCubit = getIt<MainCubit>()..getTvCredits(int.parse(widget.id));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          _detailsCubit.getTvDetails(int.parse(widget.id));
            ...
            return BlocBuilder<MainCubit, ApiState<dynamic>>(
              ...
              builder: (context, creditsState) {
                ...
                if (detailsState is ApiLoaded) {
                  tvDetailModel = detailsState.data as TvDetailModel;
                }

                if (creditsState is ApiLoaded) {
                  credits = creditsState.data as TvCastCrewModel;
                }
